...
const MyTaskClient = ({ tasks = [], proposals = [] }) => {
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-5 w-full">
                {
                    taskList.length > 0 ? (
                        taskList.map((task, i) => {
                            const pCount = Array.isArray(proposals)
                                ? proposals.filter(p => String(p?.taskId) === String(task?._id)).length
                                : 0;
                                ...
                                >
                                    ...
                                    <div className="flex flex-wrap items-center justify-between gap-3 pt-3 border-t border-zinc-800/40 w-full mt-auto">
                                        ...
                                        <div className="flex items-center gap-1 text-zinc-400 text-xs font-medium">
                                            <GoGitCommit className="w-3.5 h-3.5 rotate-90 text-zinc-500" />
                                            <span>{pCount} {pCount === 1 ? 'proposal' : 'proposals'}</span>
                                        </div>
                        ...
                        <div className="col-span-full py-12 text-center text-zinc-500 font-medium">
                            No tasks posted yet. Create your first task to get started!
                        </div>
                    )
                }
            </div>
